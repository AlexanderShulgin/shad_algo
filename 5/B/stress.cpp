...
void TraceHistory(std::vector<std::vector<std::vector<Vertex>>>& history,
                  Vertex end) {
    std::cerr << "row "<<end.row<<"column "<<end.column<<"code "<<end.code<<"\n";
...
void StressTest() {
    ...
    for (int iter = 1; iter <= 1000; ++iter) {  // NOLINT
        std::cerr << "Test " << iter << "... ";
        auto array = GenRandomArray(&generator, 2, 1, 1000);  // NOLINT

        short number_of_rows = array[0];
        short number_of_columns = array[1];

        std::vector<std::vector<char>> matrix(
            number_of_rows, std::vector<char>(number_of_columns, 'a'));
        for (short row = 0; row < number_of_rows; row++) {
            auto va = GenRandomArray(&generator, number_of_columns, 0, 5);
        ...
        Vertex begin;
        auto rows = GenRandomArray(&generator, 2, 1, number_of_rows);
        auto columns = GenRandomArray(&generator, 2, 1, number_of_columns);
        begin.row = rows[0];
        begin.column = columns[0];
        begin.row--;
        begin.column--;
        Vertex end;
        end.row = rows[1];
        ...
        if(matrix[begin.row][begin.column] == '#' || matrix[end.row][end.column] == '#'){
            continue;
        }
        ...
        std::cerr << "BEGIN"<<begin.row<<" "<<begin.column<<" "<<begin.code<<"\n";
